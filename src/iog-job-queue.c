...
static gboolean
remove_job_from_queue (GQueue *queue, IogJob *job)
{
	GList *list;
...
static void
add_job_to_queue_locked (GQueue *queue, IogJob  *job)
{
	g_object_ref (job);
...
{
	iog_job_finished (IOG_JOB (job));

	return FALSE;
}

static void
handle_job (IogJob *job)
{
	g_object_ref (G_OBJECT (job));

	// Do the IOG_JOB cast for safety
	iog_job_run (IOG_JOB (job));
...
search_for_jobs_unlocked (void)
{
	...
	job = (IogJob *) g_queue_pop_head (load_queue);
	if (job)
		return job;

	job = (IogJob *) g_queue_pop_head (transform_queue);
	if (job)
		return job;

	job = (IogJob *) g_queue_pop_head (thumbnail_queue);
	if (job)
		return job;

	job = (IogJob *) g_queue_pop_head (model_queue);
	if (job)
		return job;

	job = (IogJob *) g_queue_pop_head (save_queue);
	if (job)
		return job;

	job = (IogJob *) g_queue_pop_head (copy_queue);
	if (job)
		return job;
...
static gpointer
iog_render_thread (gpointer data)
{
	while (TRUE) {
		IogJob *job;

		g_mutex_lock (&iog_queue_mutex);

		if (no_jobs_available_unlocked ()) {
			g_cond_wait (&render_cond, &iog_queue_mutex);
		}

		job = search_for_jobs_unlocked ();

		g_mutex_unlock (&iog_queue_mutex);
...
void
iog_job_queue_init (void)
{
	g_cond_init (&render_cond);
	g_mutex_init (&iog_queue_mutex);
	...
	g_thread_new ("IogJobQueue", iog_render_thread, NULL);
}

static GQueue *
find_queue (IogJob *job)
{
	if (IOG_IS_JOB_THUMBNAIL (job)) {
		return thumbnail_queue;
	} else if (IOG_IS_JOB_LOAD (job)) {
		return load_queue;
	} else if (IOG_IS_JOB_MODEL (job)) {
		return model_queue;
	} else if (IOG_IS_JOB_TRANSFORM (job)) {
		return transform_queue;
	} else if (IOG_IS_JOB_SAVE (job)) {
		return save_queue;
	} else if (IOG_IS_JOB_COPY (job)) {
		return copy_queue;
	}
...
void
iog_job_queue_add_job (IogJob *job)
{
	GQueue *queue;
...
gboolean
iog_job_queue_remove_job (IogJob *job)
{
	gboolean retval = FALSE;

	g_return_val_if_fail (IOG_IS_JOB (job), FALSE);
