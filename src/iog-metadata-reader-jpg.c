...
#define IOG_JPEG_MARKER_START   0xFF
#define IOG_JPEG_MARKER_SOI     0xD8
#define IOG_JPEG_MARKER_APP1	0xE1
#define IOG_JPEG_MARKER_APP2	0xE2
#define IOG_JPEG_MARKER_APP14	0xED
...
struct _IogMetadataReaderJpgPrivate {
	IogMetadataReaderState  state;
...
#define IOG_METADATA_READER_JPG_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_METADATA_READER_JPG, IogMetadataReaderJpgPrivate))

static void
iog_metadata_reader_jpg_init_emr_iface (gpointer g_iface, gpointer iface_data);


G_DEFINE_TYPE_WITH_CODE (IogMetadataReaderJpg, iog_metadata_reader_jpg,
			 G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (IOG_TYPE_METADATA_READER,
			 		iog_metadata_reader_jpg_init_emr_iface))


static void
iog_metadata_reader_jpg_dispose (GObject *object)
{
	IogMetadataReaderJpg *emr = IOG_METADATA_READER_JPG (object);
	...
	G_OBJECT_CLASS (iog_metadata_reader_jpg_parent_class)->dispose (object);
}

static void
iog_metadata_reader_jpg_init (IogMetadataReaderJpg *obj)
{
	IogMetadataReaderJpgPrivate *priv;

	priv = obj->priv =  IOG_METADATA_READER_JPG_GET_PRIVATE (obj);
	priv->exif_chunk = NULL;
	priv->exif_len = 0;
	priv->iptc_chunk = NULL;
	priv->iptc_len = 0;
...
static void
iog_metadata_reader_jpg_class_init (IogMetadataReaderJpgClass *klass)
{
	GObjectClass *object_class = (GObjectClass*) klass;

	object_class->dispose = iog_metadata_reader_jpg_dispose;

	g_type_class_add_private (klass, sizeof (IogMetadataReaderJpgPrivate));
}

static gboolean
iog_metadata_reader_jpg_finished (IogMetadataReaderJpg *emr)
{
	g_return_val_if_fail (IOG_IS_METADATA_READER_JPG (emr), TRUE);

	return (emr->priv->state == EMR_FINISHED);
}


static IogJpegApp1Type
iog_metadata_identify_app1 (gchar *buf, guint len)
{
 	if (len < 5) {
 		return EJA_OTHER;
 	}
...
static void
iog_metadata_reader_get_next_block (IogMetadataReaderJpgPrivate* priv,
				    guchar *chunk,
				    int* i,
				    const guchar *buf,
				    int len,
...
static void
iog_metadata_reader_jpg_consume (IogMetadataReaderJpg *emr, const guchar *buf, guint len)
{
	IogMetadataReaderJpgPrivate *priv;
 	IogJpegApp1Type app1_type;
	int i;
	IogMetadataReaderState next_state = EMR_READ;
	guchar *chunk = NULL;

	g_return_if_fail (IOG_IS_METADATA_READER_JPG (emr));
	...
	for (i = 0; (i < len) && (priv->state != EMR_FINISHED); i++) {
		...
		switch (priv->state) {
		case EMR_READ:
			if (buf[i] == IOG_JPEG_MARKER_START) {
		...
		case EMR_READ_MARKER:
			...
			/* we are reading some sort of APPxx or COM marker */
				...
				iog_debug_message (DEBUG_IMAGE_DATA, "APPx or COM Marker Found: %x", priv->last_marker);
			}
			else {
		...
		case EMR_SKIP_BYTES:
			iog_debug_message (DEBUG_IMAGE_DATA, "Skip bytes: %i", priv->size);
		...
		case EMR_READ_APP1:
			iog_debug_message (DEBUG_IMAGE_DATA, "Read APP1 data, Length: %i", priv->size);

			app1_type = iog_metadata_identify_app1 ((gchar*) &buf[i], priv->size);
			...
			if (chunk) {
				iog_metadata_reader_get_next_block (priv, chunk,
								    &i, buf,
								    len,
								    next_state);
			}
		...
		case EMR_READ_EXIF:
			iog_debug_message (DEBUG_IMAGE_DATA, "Read continuation of EXIF data, length: %i", priv->size);
			{
 				iog_metadata_reader_get_next_block (priv, priv->exif_chunk,
		...
		case EMR_READ_XMP:
			iog_debug_message (DEBUG_IMAGE_DATA, "Read continuation of XMP data, length: %i", priv->size);
			{
				iog_metadata_reader_get_next_block (priv, priv->xmp_chunk,
		...
		case EMR_READ_ICC:
			iog_debug_message (DEBUG_IMAGE_DATA,
			...
			iog_metadata_reader_get_next_block (priv,
							    priv->icc_chunk,
							    &i, buf, len,
							    EMR_READ_ICC);
 ...
 */
static void
iog_metadata_reader_jpg_get_exif_chunk (IogMetadataReaderJpg *emr, guchar **data, guint *len)
{
	IogMetadataReaderJpgPrivate *priv;

	g_return_if_fail (IOG_IS_METADATA_READER (emr));
	priv = emr->priv;
...
#ifdef HAVE_EXIF
static gpointer
iog_metadata_reader_jpg_get_exif_data (IogMetadataReaderJpg *emr)
{
	IogMetadataReaderJpgPrivate *priv;
	ExifData *data = NULL;

	g_return_val_if_fail (IOG_IS_METADATA_READER (emr), NULL);
	priv = emr->priv;
...
/* skip the signature */
#define IOG_XMP_OFFSET (29)

static gpointer
iog_metadata_reader_jpg_get_xmp_data (IogMetadataReaderJpg *emr )
{
	IogMetadataReaderJpgPrivate *priv;
	XmpPtr xmp = NULL;
	...
	if (priv->xmp_chunk != NULL) {
		xmp = xmp_new (priv->xmp_chunk+IOG_XMP_OFFSET,
			       priv->xmp_len-IOG_XMP_OFFSET);
	}
...
#ifdef HAVE_LCMS
static gpointer
iog_metadata_reader_jpg_get_icc_profile (IogMetadataReaderJpg *emr)
{
	IogMetadataReaderJpgPrivate *priv;
	...
	if (priv->icc_chunk) {
		...
		if (profile) {
			iog_debug_message (DEBUG_LCMS, "JPEG has ICC profile");
		} else {
			iog_debug_message (DEBUG_LCMS, "JPEG has invalid ICC profile");
		}
	}
	...
	if (!profile && priv->exif_chunk != NULL) {
		...
		ExifByteOrder o;
		gint color_space;
		ExifData *exif = iog_metadata_reader_jpg_get_exif_data (emr);
		...
		switch (color_space) {
		case 1:
			iog_debug_message (DEBUG_LCMS, "JPEG is sRGB");

			profile = cmsCreate_sRGBProfile ();

			break;
		case 2:
			iog_debug_message (DEBUG_LCMS, "JPEG is Adobe RGB (Disabled)");
		...
		case 0xFFFF:
			...
			if (entry && entry->components == 2) {
				...
				iog_debug_message (DEBUG_LCMS, "No whitepoint found");
				break;
			}
			...
			if (entry && entry->components == 6) {
				...
				iog_debug_message (DEBUG_LCMS, "No primary chromaticities found");
				break;
			}
			...
			if (entry) {
				...
				iog_debug_message (DEBUG_LCMS, "No gamma found");
				gammaValue = 2.2;
			}
			...
			iog_debug_message (DEBUG_LCMS, "JPEG is calibrated");
...
iog_metadata_reader_jpg_init_emr_iface (gpointer g_iface, gpointer iface_data)
{
	IogMetadataReaderInterface *iface;

	iface = (IogMetadataReaderInterface*)g_iface;

	iface->consume =
		(void (*) (IogMetadataReader *self, const guchar *buf, guint len))
			iog_metadata_reader_jpg_consume;
	iface->finished =
		(gboolean (*) (IogMetadataReader *self))
			iog_metadata_reader_jpg_finished;
	iface->get_raw_exif =
		(void (*) (IogMetadataReader *self, guchar **data, guint *len))
			iog_metadata_reader_jpg_get_exif_chunk;
#ifdef HAVE_EXIF
	iface->get_exif_data =
		(gpointer (*) (IogMetadataReader *self))
			iog_metadata_reader_jpg_get_exif_data;
#endif
#ifdef HAVE_LCMS
	iface->get_icc_profile =
		(gpointer (*) (IogMetadataReader *self))
			iog_metadata_reader_jpg_get_icc_profile;
#endif
#ifdef HAVE_EXEMPI
	iface->get_xmp_ptr =
		(gpointer (*) (IogMetadataReader *self))
			iog_metadata_reader_jpg_get_xmp_data;
#endif
}
