...
#define IOG_CSS_FILE_PATH IOG_DATA_DIR G_DIR_SEPARATOR_S "iog.css"
#endif

static IogStartupFlags flags;
...
_print_version_and_exit (const gchar *option_name,
			 ...
			 GError **error)
{
	g_print("%s %s\n", _("Eye of GDE2 Image Viewer"), VERSION);
	exit (EXIT_SUCCESS);
	return TRUE;
}
...
{
  if (fullscreen)
    flags |= IOG_STARTUP_FULLSCREEN;

  if (disable_collection)
    flags |= IOG_STARTUP_DISABLE_COLLECTION;

  if (slide_show)
    flags |= IOG_STARTUP_SLIDE_SHOW;
}
...
load_files (void)
{
	...
	files = iog_util_string_array_to_list ((const gchar **) startup_files, TRUE);
...
load_files_remote (void)
{
	...
	if (connection == NULL) {
 	...
 	files = iog_util_string_array_make_absolute (startup_files);

 	remote_object = dbus_g_proxy_new_for_name (connection,
 						   "org.gde2.iog.ApplicationService",
						   "/org/gde2/iog/Iog",
						   "org.gde2.iog.Application");
...
main (int argc, char **argv)
{
	...
	bindtextdomain (GETTEXT_PACKAGE, IOG_LOCALE_DIR);
	bind_textdomain_codeset (GETTEXT_PACKAGE, "UTF-8");
	textdomain (GETTEXT_PACKAGE);

#if !GTK_CHECK_VERSION (3, 0, 0)
	gtk_rc_parse (IOG_DATA_DIR G_DIR_SEPARATOR_S "gtkrc");
#endif
	...
	if (!g_option_context_parse (ctx, &argc, &argv, &error)) {
		...
		/* I18N: The '%s' is replaced with iog's command name. */
	...
	if (!force_new_instance &&
	    !iog_application_register_service (IOG_APP)) {
...
#if GTK_CHECK_VERSION (3, 0, 0)
	/* Load special style properties for IogThumbView's scrollbar */
	...
	iog_plugin_engine_init ();

	/* Add application specific icons to search path */
	gtk_icon_theme_append_search_path (gtk_icon_theme_get_default (),
                                           IOG_DATA_DIR G_DIR_SEPARATOR_S "icons");

	gtk_window_set_default_icon_name ("iog");
	g_set_application_name (_("Eye of GDE2 Image Viewer"));
