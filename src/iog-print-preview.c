...
#define IOG_PRINT_PREVIEW_GET_PRIVATE(object)				\
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_PRINT_PREVIEW, IogPrintPreviewPrivate))

G_DEFINE_TYPE (IogPrintPreview, iog_print_preview, GTK_TYPE_ASPECT_FRAME)

struct _IogPrintPreviewPrivate {
	GtkWidget *area;
	GdkPixbuf *image;
...
static void iog_print_preview_draw (IogPrintPreview *preview, cairo_t *cr);
static void iog_print_preview_finalize (GObject *object);
static void update_relative_sizes (IogPrintPreview *preview);
static void create_surface (IogPrintPreview *preview);
static void create_image_scaled (IogPrintPreview *preview);
static gboolean create_surface_when_idle (IogPrintPreview *preview);

static void
iog_print_preview_get_property (GObject    *object,
				guint       prop_id,
				GValue     *value,
				GParamSpec *pspec)
{
	IogPrintPreviewPrivate *priv = IOG_PRINT_PREVIEW (object)->priv;
...
static void
iog_print_preview_set_property (GObject      *object,
				guint         prop_id,
				const GValue *value,
				GParamSpec   *pspec)
{
	...
	update_relative_sizes (IOG_PRINT_PREVIEW (object));
	gtk_widget_queue_draw (priv->area);
}

static void
iog_print_preview_class_init (IogPrintPreviewClass *klass)
{
	GObjectClass *gobject_class;

	gobject_class = (GObjectClass*) klass;

	gobject_class->get_property = iog_print_preview_get_property;
	gobject_class->set_property = iog_print_preview_set_property;
	gobject_class->finalize     = iog_print_preview_finalize;

/**
 * IogPrintPreview:image:
 *
...
/**
 * IogPrintPreview:page-left-margin:
 *
...
/**
 * IogPrintPreview:page-right-margin:
 *
	...
	g_object_class_install_property (gobject_class,
					 ...
					 g_param_spec_float ("page-right-margin",
							     "Right margin of the page in inches",
							      ...
							      G_PARAM_READWRITE));
/**
 * IogPrintPreview:page-top-margin:
 *
...
/**
 * IogPrintPreview:page-bottom-margin:
 *
...
/**
 * IogPrintPreview::image-moved:
 * @preview: the object which received the signal
 *
 * The #IogPrintPreview::image-moved signal is emitted when the position
 ...
 */
	...
	g_type_class_add_private (klass, sizeof (IogPrintPreviewPrivate));
}

static void
iog_print_preview_finalize (GObject *object)
{
	IogPrintPreviewPrivate *priv;

	priv = IOG_PRINT_PREVIEW (object)->priv;
	...
	G_OBJECT_CLASS (iog_print_preview_parent_class)->finalize (object);
}

static void
iog_print_preview_init (IogPrintPreview *preview)
{
	IogPrintPreviewPrivate *priv;
	gfloat ratio;

	priv = preview->priv = IOG_PRINT_PREVIEW_GET_PRIVATE (preview);
...
/**
 * iog_print_preview_new_with_pixbuf:
 * @pixbuf: a #GdkPixbuf
 *
 * Creates a new #IogPrintPreview widget, and sets the #GdkPixbuf to preview
 * on it.
 *
 * Returns: A new #IogPrintPreview widget.
 **/
GtkWidget *
iog_print_preview_new_with_pixbuf (GdkPixbuf *pixbuf)
{
	IogPrintPreview *preview;

	g_return_val_if_fail (GDK_IS_PIXBUF (pixbuf), NULL);

	preview = IOG_PRINT_PREVIEW (iog_print_preview_new ());
...
/**
 * iog_print_preview_new:
 *
 * Creates a new #IogPrintPreview widget, setting it to the default values,
 * and leaving the page empty. You still need to set the #IogPrintPreview:image
 * property to make it useful.
 *
 * Returns: A new and empty #IogPrintPreview widget.
 **/
GtkWidget *
iog_print_preview_new (void)
{
	IogPrintPreview *preview;
	GtkWidget *area;

	preview = g_object_new (IOG_TYPE_PRINT_PREVIEW, NULL);
...
expose_event_cb (GtkDrawingArea *drawing_area,
		 ...
		 gpointer  user_data)
{
	update_relative_sizes (IOG_PRINT_PREVIEW (user_data));
	...
	iog_print_preview_draw (IOG_PRINT_PREVIEW (user_data), cr);
 ...
 **/
static void
get_current_image_coordinates (IogPrintPreview *preview,
			       gint *x0, gint *y0)
{
	IogPrintPreviewPrivate *priv;
 ...
 **/
static gboolean
press_inside_image_area (IogPrintPreview *preview,
			 guint x,
			 guint y)
{
	IogPrintPreviewPrivate *priv;
...
static void
create_image_scaled (IogPrintPreview *preview)
{
	IogPrintPreviewPrivate *priv = preview->priv;
...
static GdkPixbuf *
create_preview_buffer (IogPrintPreview *preview)
{
	GdkPixbuf *pixbuf;
	gint width, height;
...
static void
create_surface (IogPrintPreview *preview)
{
	IogPrintPreviewPrivate *priv = preview->priv;
	GdkPixbuf *pixbuf;
...
static gboolean
create_surface_when_idle (IogPrintPreview *preview)
{
...
button_press_event_cb (GtkWidget *widget,
		       ...
		       gpointer user_data)
{
	IogPrintPreview *preview = IOG_PRINT_PREVIEW (user_data);
...
motion_notify_event_cb (GtkWidget      *widget,
			...
			gpointer        user_data)
{
	IogPrintPreviewPrivate *priv = IOG_PRINT_PREVIEW (user_data)->priv;
	gdouble dx, dy;
	GtkAllocation allocation;
	...
	if (priv->grabbed) {
		...
		g_object_set (IOG_PRINT_PREVIEW (user_data),
		...
		if (press_inside_image_area (IOG_PRINT_PREVIEW (user_data), event->x, event->y)) {
		  	GdkCursor *cursor;
		  ...
		  gpointer user_data)
{
	...
	preview = IOG_PRINT_PREVIEW (user_data);
	update_relative_sizes (preview);
...
static void
iog_print_preview_draw (IogPrintPreview *preview, cairo_t *cr)
{
	IogPrintPreviewPrivate *priv;
	GtkWidget *area;
	GtkAllocation allocation;
...
static void
update_relative_sizes (IogPrintPreview *preview)
{
	IogPrintPreviewPrivate *priv;
	GtkAllocation allocation;
	gint i_width, i_height;
...
/**
 * iog_print_preview_set_page_margins:
 * @preview: a #IogPrintPreview
 ...
 **/
void
iog_print_preview_set_page_margins (IogPrintPreview *preview,
				    gfloat l_margin,
				    gfloat r_margin,
				    gfloat t_margin,
				    gfloat b_margin)
{
	g_return_if_fail (IOG_IS_PRINT_PREVIEW (preview));
...
/**
 * iog_print_preview_set_from_page_setup:
 * @preview: a #IogPrintPreview
 ...
 **/
void
iog_print_preview_set_from_page_setup (IogPrintPreview *preview,
				       GtkPageSetup *setup)
{
	g_return_if_fail (IOG_IS_PRINT_PREVIEW (preview));
...
/**
 * iog_print_preview_get_image_position:
 * @preview: a #IogPrintPreview
 ...
 **/
void
iog_print_preview_get_image_position (IogPrintPreview *preview,
				      gdouble *x,
				      gdouble *y)
{
	IogPrintPreviewPrivate *priv;
...
/**
 * iog_print_preview_set_image_position:
 * @preview: a #IogPrintPreview
 ...
 **/
void
iog_print_preview_set_image_position (IogPrintPreview *preview,
				      gdouble x,
				      gdouble y)
{
	IogPrintPreviewPrivate *priv;
 ...
 **/
void
iog_print_preview_set_scale (IogPrintPreview *preview,
			     gfloat           scale)
{
	g_return_if_fail (IOG_IS_PRINT_PREVIEW (preview));
