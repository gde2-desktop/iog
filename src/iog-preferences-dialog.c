...
#define IOG_PREFERENCES_DIALOG_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_PREFERENCES_DIALOG, IogPreferencesDialogPrivate))

G_DEFINE_TYPE (IogPreferencesDialog, iog_preferences_dialog, IOG_TYPE_DIALOG);

#define GSETTINGS_OBJECT_KEY		"GSETTINGS_KEY"
#define GSETTINGS_OBJECT_VALUE		"GSETTINGS_VALUE"

struct _IogPreferencesDialogPrivate {
...
pd_transp_radio_toggle_cb (GtkWidget *widget, gpointer data)
{
	...
	g_settings_set_enum (G_SETTINGS (data), IOG_CONF_VIEW_TRANSPARENCY,
...
static void
iog_preferences_response_cb (GtkDialog *dlg, gint res_id, gpointer data)
{
	switch (res_id) {
		case GTK_RESPONSE_HELP:
			iog_util_show_help ("iog-prefs", NULL);
			break;
		default:
...
static GObject *
iog_preferences_dialog_constructor (GType type,
...

{
	...
	object = G_OBJECT_CLASS (iog_preferences_dialog_parent_class)->constructor
			(type, n_construct_properties, construct_params);

	priv = IOG_PREFERENCES_DIALOG (object)->priv;

	priv->view_settings = g_settings_new (IOG_CONF_VIEW);
	priv->fullscreen_settings = g_settings_new (IOG_CONF_FULLSCREEN);

	iog_dialog_construct (IOG_DIALOG (object),
			      "iog-preferences-dialog.ui",
			      "iog_preferences_dialog");

	iog_dialog_get_controls (IOG_DIALOG (object),
			         "iog_preferences_dialog", &dlg,
	...
	g_object_set_data (G_OBJECT (color_radio),
			   ...
			   GINT_TO_POINTER (IOG_TRANSP_COLOR));
	...
	g_object_set_data (G_OBJECT (checkpattern_radio),
			   GSETTINGS_OBJECT_VALUE,
			   GINT_TO_POINTER (IOG_TRANSP_CHECKED));
	...
	g_object_set_data (G_OBJECT (background_radio),
			   GSETTINGS_OBJECT_VALUE,
			   GINT_TO_POINTER (IOG_TRANSP_BACKGROUND));
	...
	case IOG_TRANSP_CHECKED:
		...
		break;
	default:
		// Log a warning and use IOG_TRANSP_BACKGROUND as fallback
		g_warn_if_reached ();
	...
	g_settings_bind (priv->fullscreen_settings, IOG_CONF_FULLSCREEN_UPSCALE,
	...
	g_signal_connect (priv->fullscreen_settings,
					  "changed::" IOG_CONF_FULLSCREEN_RANDOM,
	...
	g_settings_bind (priv->fullscreen_settings,
        ...
        plugin_manager = iog_plugin_manager_new ();
...
static void
iog_preferences_dialog_dispose (GObject *object)
{
	IogPreferencesDialog *pref_dlg = IOG_PREFERENCES_DIALOG (object);
	...
	G_OBJECT_CLASS (iog_preferences_dialog_parent_class)->dispose (object);
}

static void
iog_preferences_dialog_class_init (IogPreferencesDialogClass *class)
{
	GObjectClass *g_object_class = (GObjectClass *) class;

	g_object_class->constructor = iog_preferences_dialog_constructor;
	g_object_class->dispose = iog_preferences_dialog_dispose;

	g_type_class_add_private (g_object_class, sizeof (IogPreferencesDialogPrivate));
}

static void
iog_preferences_dialog_init (IogPreferencesDialog *pref_dlg)
{
	pref_dlg->priv = IOG_PREFERENCES_DIALOG_GET_PRIVATE (pref_dlg);

	pref_dlg->priv->view_settings = g_settings_new (IOG_CONF_VIEW);
	pref_dlg->priv->fullscreen_settings = g_settings_new (IOG_CONF_FULLSCREEN);
	pref_dlg->priv->ui_settings = g_settings_new (IOG_CONF_UI);
}

GObject *
iog_preferences_dialog_get_instance (GtkWindow *parent)
{
	if (instance == NULL) {
		instance = g_object_new (IOG_TYPE_PREFERENCES_DIALOG,
