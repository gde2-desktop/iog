...
update_preview (gpointer user_data)
{
	...
	if (token_str != NULL) {
		...
		preview_str = iog_uri_converter_preview (token_str, data->image, format,
...
GtkWidget*
iog_save_as_dialog_new (GtkWindow *main, GList *images, GFile *base_file)
{
	char *filepath;
	GtkBuilder  *xml;
	GtkWidget *dlg;
	...
	filepath = g_build_filename (IOG_DATA_DIR,
	...
	dlg = GTK_WIDGET (g_object_ref (gtk_builder_get_object (xml, "iog_multiple_save_as_dialog")));
	...
	gtk_label_set_text (GTK_LABEL (label), iog_image_get_caption (data->image));
...
IogURIConverter*
iog_save_as_dialog_get_converter (GtkWidget *dlg)
{
	...
	conv = iog_uri_converter_new (base_file, format, format_str);
