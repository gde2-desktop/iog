...
struct _IogCloseConfirmationDialogPrivate 
{
...
#define IOG_CLOSE_CONFIRMATION_DIALOG_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), \
							IOG_TYPE_CLOSE_CONFIRMATION_DIALOG, \
							IogCloseConfirmationDialogPrivate))
...
G_DEFINE_TYPE(IogCloseConfirmationDialog, iog_close_confirmation_dialog, GTK_TYPE_DIALOG)

static void 	 set_unsaved_image 		(IogCloseConfirmationDialog *dlg,
						 const GList                  *list);

static GList 	*get_selected_imgs 		(GtkTreeModel                 *store);

static GdkPixbuf *
iog_close_confirmation_dialog_get_icon (const gchar *icon_name)
{
	GError *error = NULL;
...
get_nothumb_pixbuf (void)
{
	...
	g_once (&nothumb_once, (GThreadFunc) iog_close_confirmation_dialog_get_icon, "image-x-generic");
 ...
 */
static void
response_cb (IogCloseConfirmationDialog *dlg,
             gint                        response_id,
             gpointer                    data)
{
	IogCloseConfirmationDialogPrivate *priv;

	g_return_if_fail (IOG_IS_CLOSE_CONFIRMATION_DIALOG (dlg));
...
static void
add_buttons (IogCloseConfirmationDialog *dlg)
{
...
static void 
iog_close_confirmation_dialog_init (IogCloseConfirmationDialog *dlg)
{
	AtkObject *atk_obj;

	dlg->priv = IOG_CLOSE_CONFIRMATION_DIALOG_GET_PRIVATE (dlg);
...
static void 
iog_close_confirmation_dialog_finalize (GObject *object)
{
	IogCloseConfirmationDialogPrivate *priv;

	priv = IOG_CLOSE_CONFIRMATION_DIALOG (object)->priv;
	...
	G_OBJECT_CLASS (iog_close_confirmation_dialog_parent_class)->finalize (object);
}

static void
iog_close_confirmation_dialog_set_property (GObject      *object, 
					      guint         prop_id, 
					      const GValue *value, 
					      GParamSpec   *pspec)
{
	IogCloseConfirmationDialog *dlg;

	dlg = IOG_CLOSE_CONFIRMATION_DIALOG (object);
...
static void
iog_close_confirmation_dialog_get_property (GObject    *object, 
					      guint       prop_id, 
					      GValue     *value, 
					      GParamSpec *pspec)
{
...
static void 
iog_close_confirmation_dialog_class_init (IogCloseConfirmationDialogClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->set_property = iog_close_confirmation_dialog_set_property;
	gobject_class->get_property = iog_close_confirmation_dialog_get_property;
	gobject_class->finalize = iog_close_confirmation_dialog_finalize;

	g_type_class_add_private (klass, sizeof (IogCloseConfirmationDialogPrivate));
...
GList *
iog_close_confirmation_dialog_get_selected_images (IogCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (IOG_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);

	return g_list_copy (dlg->priv->selected_images);
}

GtkWidget *
iog_close_confirmation_dialog_new (GtkWindow *parent, 
				   GList     *unsaved_images)
{
	GtkWidget *dlg;
	GtkWindowGroup *wg;
	...
	dlg = GTK_WIDGET (g_object_new (IOG_TYPE_CLOSE_CONFIRMATION_DIALOG,
...
GtkWidget *
iog_close_confirmation_dialog_new_single (GtkWindow     *parent, 
					  IogImage	*image)
{
	GtkWidget *dlg;
	GList *unsaved_images;
	...
	dlg = iog_close_confirmation_dialog_new (parent, 
						 unsaved_images);
...
static gchar *
get_text_secondary_label (IogImage *image)
{
	gchar *secondary_msg;
...
static void
build_single_img_dialog (IogCloseConfirmationDialog *dlg)
{
	GtkWidget     *hbox;
	GtkWidget     *vbox;
	...
	img = IOG_IMAGE (dlg->priv->unsaved_images->data);
	...
	image_name = iog_image_get_caption (img);
...
populate_model (GtkTreeModel *store, GList *imgs)
{
	...
	while (imgs != NULL)
	{
		...
		img = IOG_IMAGE (imgs->data);

		name = iog_image_get_caption (img);
		buf = iog_image_get_thumbnail (img);
...
static GtkWidget *
create_treeview (IogCloseConfirmationDialogPrivate *priv)
{
	GtkListStore *store;
	GtkWidget *treeview;
...
static void
build_multiple_imgs_dialog (IogCloseConfirmationDialog *dlg)
{
	IogCloseConfirmationDialogPrivate *priv;
	GtkWidget *hbox;
	GtkWidget *image;
...
static void
set_unsaved_image (IogCloseConfirmationDialog *dlg,
		   const GList                *list)
{
	IogCloseConfirmationDialogPrivate *priv;
...
const GList *
iog_close_confirmation_dialog_get_unsaved_images (IogCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (IOG_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);
...
void
iog_close_confirmation_dialog_set_sensitive (IogCloseConfirmationDialog *dlg,
					     gboolean value)
{
