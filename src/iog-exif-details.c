...
#define IOG_EXIF_DETAILS_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_EXIF_DETAILS, IogExifDetailsPrivate))

G_DEFINE_TYPE (IogExifDetails, iog_exif_details, GTK_TYPE_TREE_VIEW)
...
static void iog_exif_details_reset (IogExifDetails *exif_details);

static void
iog_exif_details_dispose (GObject *object)
{
	IogExifDetailsPrivate *priv;

	priv = IOG_EXIF_DETAILS (object)->priv;
	...
	G_OBJECT_CLASS (iog_exif_details_parent_class)->dispose (object);
}

static void
iog_exif_details_init (IogExifDetails *exif_details)
{
	IogExifDetailsPrivate *priv;
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;

	exif_details->priv = IOG_EXIF_DETAILS_GET_PRIVATE (exif_details);
	...
	iog_exif_details_reset (exif_details);
...
static void
iog_exif_details_class_init (IogExifDetailsClass *klass)
{
	GObjectClass *object_class = (GObjectClass*) klass;

	object_class->dispose = iog_exif_details_dispose;

	g_type_class_add_private (object_class, sizeof (IogExifDetailsPrivate));
}
...
set_row_data (GtkTreeStore *store, char *path, char *parent, const char *attribute, const char *value)
{
	...
	utf_attribute = iog_util_make_valid_utf8 (attribute);

	gtk_tree_store_set (store, &iter, MODEL_COLUMN_ATTRIBUTE, utf_attribute, -1);
	g_free (utf_attribute);

	if (value != NULL) {
		utf_value = iog_util_make_valid_utf8 (value);
...
GtkWidget *
iog_exif_details_new (void)
{
	GObject *object;

	object = g_object_new (IOG_TYPE_EXIF_DETAILS, NULL);

	return GTK_WIDGET (object);
}

static void
iog_exif_details_reset (IogExifDetails *exif_details)
{
	int i;
	IogExifDetailsPrivate *priv = exif_details->priv;
...
#ifdef HAVE_EXIF
void
iog_exif_details_update (IogExifDetails *exif_details, ExifData *data)
{
	g_return_if_fail (IOG_IS_EXIF_DETAILS (exif_details));
...
static void
xmp_entry_insert (IogExifDetails *view, XmpStringPtr xmp_schema,
...
void
iog_exif_details_xmp_update (IogExifDetails *view, XmpPtr data)
{
	g_return_if_fail (IOG_IS_EXIF_DETAILS (view));
