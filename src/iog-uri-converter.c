...
typedef struct {
	IogUCType  type;
	union {
		char    *string;  /* if type == IOG_UC_STRING */
		gulong  counter;  /* if type == IOG_UC_COUNTER */
	} data;
} IogUCToken;


struct _IogURIConverterPrivate {
	GFile           *base_file;
	GList           *token_list;
	char            *suffix;
...
static void iog_uri_converter_set_property (GObject      *object,
					    guint         property_id,
					    const GValue *value,
					    GParamSpec   *pspec);

static void iog_uri_converter_get_property (GObject    *object,
					    guint       property_id,
					    GValue     *value,
					    GParamSpec *pspec);

#define IOG_URI_CONVERTER_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_URI_CONVERTER, IogURIConverterPrivate))

G_DEFINE_TYPE (IogURIConverter, iog_uri_converter, G_TYPE_OBJECT)

static void
free_token (gpointer data)
{
	IogUCToken *token = (IogUCToken*) data;

	if (token->type == IOG_UC_STRING) {
		g_free (token->data.string);
	}

	g_slice_free (IogUCToken, token);
}

static void
iog_uri_converter_dispose (GObject *object)
{
	IogURIConverter *instance = IOG_URI_CONVERTER (object);
	...
	G_OBJECT_CLASS (iog_uri_converter_parent_class)->dispose (object);
}

static void
iog_uri_converter_init (IogURIConverter *obj)
{
	IogURIConverterPrivate *priv;

	priv = obj->priv = IOG_URI_CONVERTER_GET_PRIVATE (obj);
...
static void
iog_uri_converter_class_init (IogURIConverterClass *klass)
{
	GObjectClass *object_class = (GObjectClass*) klass;

	object_class->dispose = iog_uri_converter_dispose;

        /* GObjectClass */
        object_class->set_property = iog_uri_converter_set_property;
        object_class->get_property = iog_uri_converter_get_property;
	...
	g_type_class_add_private (klass, sizeof (IogURIConverterPrivate));
}

GQuark
iog_uc_error_quark (void)
{
	static GQuark q = 0;
	if (q == 0)
		q = g_quark_from_static_string ("iog-uri-converter-error-quark");
...
static void
iog_uri_converter_set_property (GObject      *object,
                                guint         property_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
	IogURIConverter *conv;
	IogURIConverterPrivate *priv;

        g_return_if_fail (IOG_IS_URI_CONVERTER (object));

        conv = IOG_URI_CONVERTER (object);
	priv = conv->priv;
...
static void
iog_uri_converter_get_property (GObject    *object,
                                guint       property_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
...
create_token_string (const char *string, int substr_start, int substr_len)
{
	...
	token = g_slice_new0 (IogUCToken);
	token->type = IOG_UC_STRING;
...
static IogUCToken*
create_token_other (IogUCType type)
{
	IogUCToken *token;
...
static GList*
iog_uri_converter_parse_string (IogURIConverter *conv, const char *string)
{
	IogURIConverterPrivate *priv;
	GList *list = NULL;
	gulong len;
	...
	g_return_val_if_fail (IOG_IS_URI_CONVERTER (conv), NULL);
	...
	for (i = 0; i < len; i++) {
		...
		case PARSER_TOKEN: {
			IogUCType type = IOG_UC_END;

			if (c == 'f') {
				type = IOG_UC_FILENAME;
			}
			else if (c == 'n') {
				type = IOG_UC_COUNTER;
...
void
iog_uri_converter_print_list (IogURIConverter *conv)
{
	IogURIConverterPrivate *priv;
	GList *it;

	g_return_if_fail (IOG_URI_CONVERTER (conv));
	...
	for (it = priv->token_list; it != NULL; it = it->next) {
		...
		if (token->type == IOG_UC_STRING || token->type == IOG_UC_COUNTER) {
			g_free (str);
		}
	}
}
...
IogURIConverter*
iog_uri_converter_new (GFile *base_file, GdkPixbufFormat *img_format, const char *format_str)
{
	IogURIConverter *conv;

	g_return_val_if_fail (format_str != NULL, NULL);

	conv = g_object_new (IOG_TYPE_URI_CONVERTER, NULL);
	...
	conv->priv->token_list = iog_uri_converter_parse_string (conv, format_str);

	return conv;
}

static GFile*
get_file_directory (IogURIConverter *conv, IogImage *image)
{
	GFile *file = NULL;
	IogURIConverterPrivate *priv;
	...
	if (priv->base_file != NULL) {
		...
		img_file = iog_image_get_file (image);
...
static GString*
append_filename (GString *str, IogImage *img)
{
	...
	img_file = iog_image_get_file (img);
...
static GString*
append_counter (GString *str, gulong counter,  IogURIConverter *conv)
{
	IogURIConverterPrivate *priv;
...
static void
build_absolute_file (IogURIConverter *conv, IogImage *image, GString *str,  /* input  */
		     GFile **file, GdkPixbufFormat **format)                /* output */
{
	...
	*file = NULL;
	...
	g_return_if_fail (IOG_IS_URI_CONVERTER (conv));
	...
	if (priv->img_format == NULL) {
		...
		if (format != NULL)
			*format = iog_pixbuf_get_format_by_suffix (old_suffix);

		g_object_unref (img_file);
	} else {
		if (priv->suffix == NULL)
			priv->suffix = iog_pixbuf_get_common_suffix (priv->img_format);
...
/*
 * This function converts the uri of the IogImage object, according to the
 * IogUCToken list. The absolute uri (converted filename appended to base uri)
 * is returned in uri and the image format will be in the format pointer.
 */
gboolean
iog_uri_converter_do (IogURIConverter *conv, IogImage *image,
		      GFile **file, GdkPixbufFormat **format, GError **error)
{
	...
	g_return_val_if_fail (IOG_IS_URI_CONVERTER (conv), FALSE);
	...
	*file = NULL;
	...
	for (it = priv->token_list; it != NULL; it = it->next) {
		IogUCToken *token = (IogUCToken*) it->data;
...
char*
iog_uri_converter_preview (const char *format_str, IogImage *img, GdkPixbufFormat *format,
			   ...
			   gboolean convert_spaces, gunichar space_char)
{
	...
	if (repl_str->len > 0) {
		if (format == NULL) {
			...
			char *suffix = iog_pixbuf_get_common_suffix (format);
...
gboolean
iog_uri_converter_requires_exif (IogURIConverter *converter)
{
	g_return_val_if_fail (IOG_IS_URI_CONVERTER (converter), FALSE);

	return converter->priv->requires_exif;
}

gboolean
iog_uri_converter_check (IogURIConverter *converter, GList *img_list, GError **error)
{
	GList *it;
	...
	for (it = img_list; it != NULL; it = it->next) {
		...
		result = iog_uri_converter_do (converter, IOG_IMAGE (it->data),
	...
	if (!all_different) {
		g_set_error (error, IOG_UC_ERROR,
