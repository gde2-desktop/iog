...
typedef enum {
	IOG_SAVE_NONE,
	IOG_SAVE_JPEG_AS_JPEG,
	IOG_SAVE_ANY_AS_JPEG
} IogJpegSaveMethod;
...
static void
init_transform_info (IogImage *image, jpeg_transform_info *info)
{
	IogImagePrivate *priv;
	IogTransform *composition = NULL;
	IogTransformType transformation;
	...
	if (composition != NULL) {
		transformation = iog_transform_get_transform_type (composition);
		...
		case IOG_TRANSFORM_TRANSVERSE:
			...
			break;
		default:
			g_warning("IogTransformType not supported!");
...
static gboolean
_save_jpeg_as_jpeg (IogImage *image, const char *file, IogImageSaveInfo *source,
		    IogImageSaveInfo *target, GError **error)
{
	...
	g_return_val_if_fail (IOG_IMAGE (image)->priv->file != NULL, FALSE);
...
static gboolean
_save_any_as_jpeg (IogImage *image, const char *file, IogImageSaveInfo *source,
		   IogImageSaveInfo *target, GError **error)
{
	IogImagePrivate *priv;
	GdkPixbuf *pixbuf;
	...
	g_return_val_if_fail (IOG_IS_IMAGE (image), FALSE);
	g_return_val_if_fail (IOG_IMAGE (image)->priv->image != NULL, FALSE);
...
gboolean
iog_image_jpeg_save_file (IogImage *image, const char *file,
			  IogImageSaveInfo *source, IogImageSaveInfo *target,
			  GError **error)
{
	IogJpegSaveMethod method = IOG_SAVE_NONE;
	...
	source_is_jpeg = !g_ascii_strcasecmp (source->format, IOG_FILE_FORMAT_JPEG);

	/* determine which method should be used for saving */
	if (target == NULL) {
		if (source_is_jpeg) {
			method = IOG_SAVE_JPEG_AS_JPEG;
		}
	}
	else {
		target_is_jpeg = !g_ascii_strcasecmp (target->format, IOG_FILE_FORMAT_JPEG);
