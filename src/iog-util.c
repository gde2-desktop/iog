...
void
iog_util_show_help (const gchar *section, GtkWindow *parent)
{
	GError *error = NULL;
	gchar *uri = NULL;

	if (section)
		uri = g_strdup_printf ("help:iog/%s", section);

	gtk_show_uri (NULL, ((uri != NULL) ? uri : "help:iog"),
	...
	if (error) {
		...
		dialog = gtk_message_dialog_new (parent,
						 ...
						 GTK_MESSAGE_ERROR,
						 GTK_BUTTONS_OK,
						 _("Could not display help for Eye of GDE2"));
...
gchar *
iog_util_make_valid_utf8 (const gchar *str)
{
	GString *string;
...
GSList*
iog_util_parse_uri_string_list_to_file_list (const gchar *uri_list)
{
...
GSList*
iog_util_string_list_to_file_list (GSList *string_list)
{
	GSList *it = NULL;
	GSList *file_list = NULL;
...
#ifdef HAVE_DBUS
GSList*
iog_util_strings_to_file_list (gchar **strings)
{
	int i;
 	GSList *file_list = NULL;
...
GSList*
iog_util_string_array_to_list (const gchar **files, gboolean create_uri)
{
	gint i;
	GSList *list = NULL;
...
gchar **
iog_util_string_array_make_absolute (gchar **files)
{
	int i;
	int size;
	gchar **abs_files;
...
const gchar *
iog_util_dot_dir (void)
{
	if (dot_dir == NULL) {
		gboolean exists;

			dot_dir = g_build_filename(g_get_user_config_dir(), "gde2", "iog", NULL);
		...
		if (G_UNLIKELY (!exists)) {
			...
			if (!printed_warning) {
				g_warning ("IOG could not save some of your preferences in its settings directory due to a file with the same name (%s) blocking its creation. Please remove that file, or move it away.", dot_dir);
...
/**
 * iog_util_filename_get_extension:
 * @filename: a filename
 *
 ...
 **/
char *
iog_util_filename_get_extension (const char * filename)
{
	char *begin, *begin2;
 ...
 **/
gboolean
iog_util_file_is_persistent (GFile *file)
{
	GMount *mount;
