...
#define IOG_PYTHON_MODULE_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_PYTHON_MODULE, IogPythonModulePrivate))

struct _IogPythonModulePrivate {
	gchar *module;
	gchar *path;
	GType type;
};
...
extern PyMethodDef pyiog_functions[];

static PyTypeObject *PyIogPlugin_Type;

G_DEFINE_TYPE (IogPythonModule, iog_python_module, G_TYPE_TYPE_MODULE)

static gboolean
iog_python_module_load (GTypeModule *gmodule)
{
	IogPythonModulePrivate *priv = IOG_PYTHON_MODULE_GET_PRIVATE (gmodule);
	...
	while (PyDict_Next (locals, &pos, &key, &value)) {
		...
		if (PyObject_IsSubclass (value, (PyObject*) PyIogPlugin_Type)) {
			priv->type = iog_python_plugin_get_type (gmodule, value);
			return TRUE;
		}
	}
...
static void
iog_python_module_unload (GTypeModule *module)
{
	IogPythonModulePrivate *priv = IOG_PYTHON_MODULE_GET_PRIVATE (module);

	iog_debug_message (DEBUG_PLUGINS, "Unloading Python module");

	priv->type = 0;
}

GObject *
iog_python_module_new_object (IogPythonModule *module)
{
	IogPythonModulePrivate *priv = IOG_PYTHON_MODULE_GET_PRIVATE (module);

	iog_debug_message (DEBUG_PLUGINS, "Creating object of type %s", g_type_name (priv->type));
...
static void
iog_python_module_init (IogPythonModule *module)
{
	iog_debug_message (DEBUG_PLUGINS, "Init of Python module");
}

static void
iog_python_module_finalize (GObject *object)
{
	IogPythonModulePrivate *priv = IOG_PYTHON_MODULE_GET_PRIVATE (object);

	iog_debug_message (DEBUG_PLUGINS, "Finalizing Python module %s", g_type_name (priv->type));

	g_free (priv->module);
	g_free (priv->path);

	G_OBJECT_CLASS (iog_python_module_parent_class)->finalize (object);
}

static void
iog_python_module_get_property (GObject    *object,
				guint       prop_id,
				GValue     *value,
				GParamSpec *pspec)
{
...
static void
iog_python_module_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *pspec)
{
	IogPythonModule *mod = IOG_PYTHON_MODULE (object);

	switch (prop_id) {
	case PROP_MODULE:
		IOG_PYTHON_MODULE_GET_PRIVATE (mod)->module = g_value_dup_string (value);
		break;

	case PROP_PATH:
		IOG_PYTHON_MODULE_GET_PRIVATE (mod)->path = g_value_dup_string (value);
		break;
...
static void
iog_python_module_class_init (IogPythonModuleClass *class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (class);
	GTypeModuleClass *module_class = G_TYPE_MODULE_CLASS (class);

	object_class->finalize = iog_python_module_finalize;
	object_class->get_property = iog_python_module_get_property;
	object_class->set_property = iog_python_module_set_property;
	...
	g_type_class_add_private (object_class, sizeof (IogPythonModulePrivate));

	module_class->load = iog_python_module_load;
	module_class->unload = iog_python_module_unload;
}

IogPythonModule *
iog_python_module_new (const gchar *path,
		       const gchar *module)
{
	IogPythonModule *result;

	if (module == NULL || module[0] == '\0')
		return NULL;

	result = g_object_new (IOG_TYPE_PYTHON_MODULE,
			       "module", module,
			       "path", path,
			       NULL);
...
gboolean
iog_python_init (void)
{
	PyObject *mdict, *path, *tuple;
	PyObject *sys_path, *iog;
	PyObject *gettext, *install, *gettext_args;
	struct sigaction old_sigint;
	gint res;
	/* Workaround for python bug. See #569228. */
	char *argv[] = { "/dev/null/python/is/buggy/iog", NULL };
	...
	path = PyString_FromString (IOG_PLUGIN_DIR "/");
	PyList_Insert (sys_path, 0, path);
	Py_DECREF(path);

	/* import iog */
	iog = Py_InitModule ("iog", pyiog_functions);
	mdict = PyModule_GetDict (iog);

	pyiog_register_classes (mdict);
	pyiog_add_constants (iog, "IOG_");
	...
	/* Retrieve the Python type for iog.Plugin */
	PyIogPlugin_Type = (PyTypeObject *) PyDict_GetItemString (mdict, "Plugin");

	if (PyIogPlugin_Type == NULL) {
		PyErr_Print ();
	...
	gettext_args = Py_BuildValue ("ss", GETTEXT_PACKAGE, IOG_LOCALE_DIR);
...
void
iog_python_garbage_collect (void)
{
	if (Py_IsInitialized()) {
		/*
