...
#if HAVE_METADATA
#include "iog-exif-details.h"
#endif

#define IOG_PROPERTIES_DIALOG_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_PROPERTIES_DIALOG, IogPropertiesDialogPrivate))

G_DEFINE_TYPE (IogPropertiesDialog, iog_properties_dialog, IOG_TYPE_DIALOG);
...
struct _IogPropertiesDialogPrivate {
	IogThumbView   *thumbview;

	gboolean        update_page;
	IogPropertiesDialogPage current_page;
...
static void
pd_update_general_tab (IogPropertiesDialog *prop_dlg,
		       IogImage            *image)
{
	...
	g_object_set (G_OBJECT (prop_dlg->priv->thumbnail_image),
		      "pixbuf", iog_image_get_thumbnail (image),
		      NULL);
...
#if HAVE_EXIF
static void
iog_exif_set_label (GtkWidget *w, ExifData *exif_data, gint tag_id)
{
	...
	if (exif_data) {
		buf_ptr = iog_exif_data_get_value (exif_data, tag_id,
						   exif_buffer, 512);

		if (tag_id == EXIF_TAG_DATE_TIME_ORIGINAL && buf_ptr)
			label_text = iog_exif_util_format_date (buf_ptr);
		else
			label_text = iog_util_make_valid_utf8 (buf_ptr);
	}
...
static void
iog_exif_set_focal_length_label (GtkWidget *w, ExifData *exif_data)
{
...
#if HAVE_METADATA
static void
pd_update_metadata_tab (IogPropertiesDialog *prop_dlg,
			IogImage            *image)
{
	...
	g_return_if_fail (IOG_IS_PROPERTIES_DIALOG (prop_dlg));
	...
	if (TRUE
#if HAVE_EXIF
	    && !iog_image_has_data (image, IOG_IMAGE_DATA_EXIF)
#endif
#if HAVE_EXEMPI
	    && !iog_image_has_data (image, IOG_IMAGE_DATA_XMP)
#endif
	    ) {
		if (gtk_notebook_get_current_page (notebook) ==	IOG_PROPERTIES_DIALOG_PAGE_EXIF) {
			gtk_notebook_prev_page (notebook);
		} else if (gtk_notebook_get_current_page (notebook) == IOG_PROPERTIES_DIALOG_PAGE_DETAILS) {
			gtk_notebook_set_current_page (notebook, IOG_PROPERTIES_DIALOG_PAGE_GENERAL);
		}
	...
	iog_exif_set_label (priv->exif_aperture_label,
			    exif_data, EXIF_TAG_FNUMBER);

	iog_exif_set_label (priv->exif_exposure_label,
			    exif_data, EXIF_TAG_EXPOSURE_TIME);

	iog_exif_set_focal_length_label (priv->exif_focal_label, exif_data);

	iog_exif_set_label (priv->exif_flash_label,
			    exif_data, EXIF_TAG_FLASH);

	iog_exif_set_label (priv->exif_iso_label,
			    exif_data, EXIF_TAG_ISO_SPEED_RATINGS);


	iog_exif_set_label (priv->exif_metering_label,
			    exif_data, EXIF_TAG_METERING_MODE);

	iog_exif_set_label (priv->exif_model_label,
			    exif_data, EXIF_TAG_MODEL);

	iog_exif_set_label (priv->exif_date_label,
			    exif_data, EXIF_TAG_DATE_TIME_ORIGINAL);

	iog_exif_details_update (IOG_EXIF_DETAILS (priv->exif_details),
				 exif_data);
...
#if HAVE_EXEMPI
	xmp_data = (XmpPtr) iog_image_get_xmp_info (image);

 	if (xmp_data != NULL) {
		iog_xmp_set_label (xmp_data,
				   NS_IPTC4XMP,
				   "Location",
		...
		iog_exif_details_xmp_update (IOG_EXIF_DETAILS (priv->exif_details), xmp_data);
...
pd_close_button_clicked_cb (GtkButton *button,
			    gpointer   user_data)
{
	iog_dialog_hide (IOG_DIALOG (user_data));
}

static gboolean
iog_properties_dialog_page_switch (GtkNotebook     *notebook,
				   gpointer         page,
				   guint            page_index,
				   IogPropertiesDialog *prop_dlg)
{
...
static gint
iog_properties_dialog_delete (GtkWidget   *widget,
			      GdkEventAny *event,
			      gpointer     user_data)
{
	g_return_val_if_fail (IOG_IS_PROPERTIES_DIALOG (user_data), FALSE);
...
void
iog_properties_dialog_set_netbook_mode (IogPropertiesDialog *dlg,
					gboolean enable)
{
	IogPropertiesDialogPrivate *priv;

	g_return_if_fail (IOG_IS_PROPERTIES_DIALOG (dlg));
	...
	if (enable) {
		...
		if (gtk_notebook_get_current_page (GTK_NOTEBOOK (priv->notebook)) == IOG_PROPERTIES_DIALOG_PAGE_DETAILS)
...
static void
iog_properties_dialog_set_property (GObject      *object,
				    guint         prop_id,
				    const GValue *value,
				    GParamSpec   *pspec)
{
	IogPropertiesDialog *prop_dlg = IOG_PROPERTIES_DIALOG (object);
	...
	switch (prop_id) {
		...
		case PROP_NETBOOK_MODE:
			iog_properties_dialog_set_netbook_mode (prop_dlg,
...
static void
iog_properties_dialog_get_property (GObject    *object,
				    guint       prop_id,
				    GValue     *value,
				    GParamSpec *pspec)
{
...
static void
iog_properties_dialog_dispose (GObject *object)
{
	IogPropertiesDialog *prop_dlg;
	IogPropertiesDialogPrivate *priv;

	g_return_if_fail (object != NULL);
	g_return_if_fail (IOG_IS_PROPERTIES_DIALOG (object));

	prop_dlg = IOG_PROPERTIES_DIALOG (object);
	priv = prop_dlg->priv;
	...
	G_OBJECT_CLASS (iog_properties_dialog_parent_class)->dispose (object);
}

static void
iog_properties_dialog_class_init (IogPropertiesDialogClass *class)
{
	GObjectClass *g_object_class = (GObjectClass *) class;

	g_object_class->dispose = iog_properties_dialog_dispose;
	g_object_class->set_property = iog_properties_dialog_set_property;
	g_object_class->get_property = iog_properties_dialog_get_property;
	...
	g_type_class_add_private (g_object_class, sizeof (IogPropertiesDialogPrivate));
}

static void
iog_properties_dialog_init (IogPropertiesDialog *prop_dlg)
{
	IogPropertiesDialogPrivate *priv;
	GtkWidget *dlg;
#ifndef HAVE_EXEMPI
	...
	prop_dlg->priv = IOG_PROPERTIES_DIALOG_GET_PRIVATE (prop_dlg);

	priv = prop_dlg->priv;

	priv->update_page = FALSE;

	iog_dialog_construct (IOG_DIALOG (prop_dlg),
			      "iog-image-properties-dialog.ui",
			      "iog_image_properties_dialog");

	iog_dialog_get_controls (IOG_DIALOG (prop_dlg),
			         "iog_image_properties_dialog", &dlg,
	...
	g_signal_connect (dlg,
			  "delete-event",
			  G_CALLBACK (iog_properties_dialog_delete),
			  prop_dlg);

	g_signal_connect (priv->notebook,
			  "switch-page",
			  G_CALLBACK (iog_properties_dialog_page_switch),
			  prop_dlg);
	...
	priv->exif_details = iog_exif_details_new ();
 ...
 *
 *
 * Returns: (transfer full) (type IogPropertiesDialog): a new #IogPropertiesDialog
 **/

GObject *
iog_properties_dialog_new (GtkWindow    *parent,
			   IogThumbView *thumbview,
			   ...
			   GtkAction    *previous_image_action)
{
	...
	g_return_val_if_fail (IOG_IS_THUMB_VIEW (thumbview), NULL);
	g_return_val_if_fail (GTK_IS_ACTION (next_image_action), NULL);
	g_return_val_if_fail (GTK_IS_ACTION (previous_image_action), NULL);

	prop_dlg = g_object_new (IOG_TYPE_PROPERTIES_DIALOG,
				 "parent-window", parent,
			     	 "thumbview", thumbview,
			     	 NULL);

	gtk_activatable_set_related_action (GTK_ACTIVATABLE (IOG_PROPERTIES_DIALOG (prop_dlg)->priv->next_button), next_image_action);

	gtk_activatable_set_related_action (GTK_ACTIVATABLE (IOG_PROPERTIES_DIALOG (prop_dlg)->priv->previous_button), previous_image_action);

	return prop_dlg;
}

void
iog_properties_dialog_update (IogPropertiesDialog *prop_dlg,
			      IogImage            *image)
{
	g_return_if_fail (IOG_IS_PROPERTIES_DIALOG (prop_dlg));
...
void
iog_properties_dialog_set_page (IogPropertiesDialog *prop_dlg,
