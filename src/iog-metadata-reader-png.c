...
struct _IogMetadataReaderPngPrivate {
	IogMetadataReaderPngState  state;
...
#define IOG_METADATA_READER_PNG_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_METADATA_READER_PNG, IogMetadataReaderPngPrivate))

static void
iog_metadata_reader_png_init_emr_iface (gpointer g_iface, gpointer iface_data);

G_DEFINE_TYPE_WITH_CODE (IogMetadataReaderPng, iog_metadata_reader_png,
			 G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (IOG_TYPE_METADATA_READER,
			 		iog_metadata_reader_png_init_emr_iface))

static void
iog_metadata_reader_png_dispose (GObject *object)
{
	IogMetadataReaderPng *emr = IOG_METADATA_READER_PNG (object);
	IogMetadataReaderPngPrivate *priv = emr->priv;
	...
	G_OBJECT_CLASS (iog_metadata_reader_png_parent_class)->dispose (object);
}

static void
iog_metadata_reader_png_init (IogMetadataReaderPng *obj)
{
	IogMetadataReaderPngPrivate *priv;

	priv = obj->priv =  IOG_METADATA_READER_PNG_GET_PRIVATE (obj);
	priv->icc_chunk = NULL;
	priv->icc_len = 0;
	priv->xmp_chunk = NULL;
	priv->xmp_len = 0;
...
static void
iog_metadata_reader_png_class_init (IogMetadataReaderPngClass *klass)
{
	GObjectClass *object_class = (GObjectClass*) klass;

	object_class->dispose = iog_metadata_reader_png_dispose;

	g_type_class_add_private (klass, sizeof (IogMetadataReaderPngPrivate));
}

static gboolean
iog_metadata_reader_png_finished (IogMetadataReaderPng *emr)
{
	g_return_val_if_fail (IOG_IS_METADATA_READER_PNG (emr), TRUE);
...
static void
iog_metadata_reader_png_get_next_block (IogMetadataReaderPngPrivate* priv,
				    	guchar *chunk,
					int* i,
					const guchar *buf,
					int len,
...
static void
iog_metadata_reader_png_consume (IogMetadataReaderPng *emr, const guchar *buf, guint len)
{
	IogMetadataReaderPngPrivate *priv;
 	int i;
	guint32 chunk_crc;
	...
	g_return_if_fail (IOG_IS_METADATA_READER_PNG (emr));
	...
	for (i = 0; (i < len) && (priv->state != EMR_FINISHED); i++) {
		...
		case EMR_CHECK_CRC:
			...
			iog_debug_message (DEBUG_IMAGE_DATA, "Checking CRC: Chunk: 0x%X - Target: 0x%X", chunk_crc, priv->target_crc);
		...
		case EMR_READ_XMP_ITXT:
			...
			iog_metadata_reader_png_get_next_block (priv,
							    priv->xmp_chunk,
							    &i, buf, len,
							    EMR_READ_XMP_ITXT);
...
/* skip the chunk ID */
#define IOG_XMP_OFFSET (22)

static gpointer
iog_metadata_reader_png_get_xmp_data (IogMetadataReaderPng *emr )
{
	IogMetadataReaderPngPrivate *priv;
	XmpPtr xmp = NULL;

	g_return_val_if_fail (IOG_IS_METADATA_READER_PNG (emr), NULL);
...
#define IOG_ICC_INFLATE_BUFFER_STEP 1024

/* I haven't seen ICC profiles larger than 1MB yet.
 * A maximum output buffer of 5MB should be enough. */
#define IOG_ICC_INFLATE_BUFFER_LIMIT (1024*1024*5)

static gpointer
iog_metadata_reader_png_get_icc_profile (IogMetadataReaderPng *emr)
{
	IogMetadataReaderPngPrivate *priv;
	...
	if (priv->icc_chunk) {
		...
		outbuf = g_malloc (IOG_ICC_INFLATE_BUFFER_STEP);
		zstr.next_out = outbuf;
		zstr.avail_out = IOG_ICC_INFLATE_BUFFER_STEP;
		...
		do {
			if (zstr.avail_out == 0) {
				...
				gsize new_size = zstr.total_out + IOG_ICC_INFLATE_BUFFER_STEP;

				if (G_UNLIKELY (new_size > IOG_ICC_INFLATE_BUFFER_LIMIT)) {
					/* Enforce a memory limit for the output
					 * buffer to avoid possible OOM cases */
					inflateEnd (&zstr);
					g_free (outbuf);
					iog_debug_message (DEBUG_IMAGE_DATA, "ICC profile is too large. Ignoring.");
					return NULL;
				}
		...
		if (G_UNLIKELY (z_ret != Z_STREAM_END)) {
			iog_debug_message (DEBUG_IMAGE_DATA, "Error while inflating ICC profile: %s (%d)", zstr.msg, z_ret);
			inflateEnd (&zstr);
			g_free (outbuf);
			return NULL;
		}
		...
		iog_debug_message (DEBUG_LCMS, "PNG has %s ICC profile", profile ? "valid" : "invalid");
	}

	if (!profile && priv->sRGB_chunk) {
		iog_debug_message (DEBUG_LCMS, "PNG is sRGB");
	...
	if (!profile && priv->cHRM_chunk) {
		...
		iog_debug_message (DEBUG_LCMS, "Trying to calculate color profile");
...
iog_metadata_reader_png_init_emr_iface (gpointer g_iface, gpointer iface_data)
{
	...
	iface->consume =
		(void (*) (IogMetadataReader *self, const guchar *buf, guint len))
			iog_metadata_reader_png_consume;
	iface->finished =
		(gboolean (*) (IogMetadataReader *self))
			iog_metadata_reader_png_finished;
#ifdef HAVE_LCMS
	iface->get_icc_profile =
		(cmsHPROFILE (*) (IogMetadataReader *self))
			iog_metadata_reader_png_get_icc_profile;
#endif
#ifdef HAVE_EXEMPI
	iface->get_xmp_ptr =
		(gpointer (*) (IogMetadataReader *self))
			iog_metadata_reader_png_get_xmp_data;
#endif
}
