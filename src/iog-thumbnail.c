...
/* We must define GDE2_DESKTOP_USE_UNSTABLE_API to be able
   to use Gde2DesktopThumbnail */
...
#define IOG_THUMB_ERROR iog_thumb_error_quark ()

static Gde2DesktopThumbnailFactory *factory = NULL;
static GdkPixbuf *frame = NULL;

typedef enum {
	IOG_THUMB_ERROR_VFS,
	IOG_THUMB_ERROR_GENERIC,
	IOG_THUMB_ERROR_UNKNOWN
} IogThumbError;
...
static GQuark
iog_thumb_error_quark (void)
{
	static GQuark q = 0;
	if (q == 0)
		q = g_quark_from_static_string ("iog-thumb-error-quark");
...
static GdkPixbuf*
get_valid_thumbnail (IogThumbData *data, GError **error)
{
	...
	if (data->thumb_exists) {
		...
		if (thumb != NULL && !gde2_desktop_thumbnail_is_valid (thumb, data->uri_str, data->mtime)) {
...
static GdkPixbuf *
create_thumbnail_from_pixbuf (IogThumbData *data,
			      GdkPixbuf *pixbuf,
			      GError **error)
{
	GdkPixbuf *thumb;
	...
	thumb = gde2_desktop_thumbnail_scale_down_pixbuf (pixbuf,
							   width*perc,
							   height*perc);
...
static void
iog_thumb_data_free (IogThumbData *data)
{
	if (data == NULL)
		return;
	...
	g_slice_free (IogThumbData, data);
}

static IogThumbData*
iog_thumb_data_new (GFile *file, GError **error)
{
	IogThumbData *data;
	GFileInfo *file_info;
	GError *ioerror = NULL;
	...
	data = g_slice_new0 (IogThumbData);

	data->uri_str    = g_file_get_uri (file);
	data->thumb_path = gde2_desktop_thumbnail_path_for_uri (data->uri_str, GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
...
static GdkPixbuf *
iog_thumbnail_stretch_frame_image (GdkPixbuf *frame_image,
				   gint left_offset,
				   gint top_offset,
				   gint right_offset,
				   gint bottom_offset,
 ...
 **/
GdkPixbuf *
iog_thumbnail_add_frame (GdkPixbuf *thumbnail)
{
	GdkPixbuf *result_pixbuf;
	...
	result_pixbuf = iog_thumbnail_stretch_frame_image (frame,
							   3, 3, 6, 6,
	                                	           dest_width,
							   dest_height,
							   FALSE);
 ...
 **/
GdkPixbuf *
iog_thumbnail_fit_to_size (GdkPixbuf *thumbnail, gint dimension)
{
	gint width, height;
	...
	if (width > dimension || height > dimension) {
		...
		result_pixbuf = gde2_desktop_thumbnail_scale_down_pixbuf (thumbnail, width, height);
 ...
 **/
GdkPixbuf*
iog_thumbnail_load (IogImage *image, GError **error)
{
	GdkPixbuf *thumb = NULL;
	GFile *file;
	IogThumbData *data;
	...
	data = iog_thumb_data_new (file, error);
	g_object_unref (file);
	...
	if (!data->can_read ||
	    (data->failed_thumb_exists && gde2_desktop_thumbnail_factory_has_valid_failed_thumbnail (factory, data->uri_str, data->mtime))) {
		iog_debug_message (DEBUG_THUMBNAIL, "%s: bad permissions or valid failed thumbnail present",data->uri_str);
		set_thumb_error (error, IOG_THUMB_ERROR_GENERIC, "Thumbnail creation failed");
		return NULL;
	}
	...
	if (thumb != NULL) {
		iog_debug_message (DEBUG_THUMBNAIL, "%s: loaded from cache",data->uri_str);
	} else if (gde2_desktop_thumbnail_factory_can_thumbnail (factory, data->uri_str, data->mime_type, data->mtime)) {
		/* Only use the image pixbuf when it is up to date. */
		if (!iog_image_is_file_changed (image))
			pixbuf = iog_image_get_pixbuf (image);

		if (pixbuf != NULL) {
			/* generate a thumbnail from the in-memory image,
			   if we have already loaded the image */
			iog_debug_message (DEBUG_THUMBNAIL, "%s: creating from pixbuf",data->uri_str);
			thumb = create_thumbnail_from_pixbuf (data, pixbuf, error);
			g_object_unref (pixbuf);
		} else {
			/* generate a thumbnail from the file */
			iog_debug_message (DEBUG_THUMBNAIL, "%s: creating from file",data->uri_str);
			thumb = gde2_desktop_thumbnail_factory_generate_thumbnail (factory, data->uri_str, data->mime_type);
		}

		if (thumb != NULL) {
			/* Save the new thumbnail */
			gde2_desktop_thumbnail_factory_save_thumbnail (factory, thumb, data->uri_str, data->mtime);
			iog_debug_message (DEBUG_THUMBNAIL, "%s: normal thumbnail saved",data->uri_str);
		} else {
			/* Save a failed thumbnail, to stop further thumbnail attempts */
			gde2_desktop_thumbnail_factory_create_failed_thumbnail (factory, data->uri_str, data->mtime);
			iog_debug_message (DEBUG_THUMBNAIL, "%s: failed thumbnail saved",data->uri_str);
			set_thumb_error (error, IOG_THUMB_ERROR_GENERIC, "Thumbnail creation failed");
		}
	}
...
void
iog_thumbnail_init (void)
{
	if (factory == NULL) {
		factory = gde2_desktop_thumbnail_factory_new (GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
	}

	if (frame == NULL) {
		frame = gdk_pixbuf_new_from_file (IOG_DATA_DIR "/pixmaps/thumbnail-frame.png", NULL);
	}
}
