...
#define IOG_STATUSBAR_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_STATUSBAR, IogStatusbarPrivate))
...
G_DEFINE_TYPE (IogStatusbar, iog_statusbar, GTK_TYPE_STATUSBAR)

struct _IogStatusbarPrivate
{
...
static void
iog_statusbar_class_init (IogStatusbarClass *klass)
{
	GObjectClass *g_object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (g_object_class, sizeof (IogStatusbarPrivate));
}

static void
iog_statusbar_init (IogStatusbar *statusbar)
{
	IogStatusbarPrivate *priv;
	GtkWidget *vbox;

	statusbar->priv = IOG_STATUSBAR_GET_PRIVATE (statusbar);
	priv = statusbar->priv;
...
GtkWidget *
iog_statusbar_new (void)
{
	return GTK_WIDGET (g_object_new (IOG_TYPE_STATUSBAR, NULL));
}

void
iog_statusbar_set_image_number (IogStatusbar *statusbar,
                                gint          num,
				gint          tot)
{
	gchar *msg;

	g_return_if_fail (IOG_IS_STATUSBAR (statusbar));
...
void
iog_statusbar_set_progress (IogStatusbar *statusbar,
			    gdouble       progress)
{
	g_return_if_fail (IOG_IS_STATUSBAR (statusbar));
