...
#define IOG_FILE_CHOOSER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
					     IOG_TYPE_FILE_CHOOSER,		    \
					     IogFileChooserPrivate))
...
struct _IogFileChooserPrivate
{
	Gde2DesktopThumbnailFactory *thumb_factory;
...
G_DEFINE_TYPE(IogFileChooser, iog_file_chooser, GTK_TYPE_FILE_CHOOSER_DIALOG)

static void
iog_file_chooser_finalize (GObject *object)
{
	IogFileChooserPrivate *priv;

	priv = IOG_FILE_CHOOSER (object)->priv;

	if (priv->thumb_factory != NULL)
		g_object_unref (priv->thumb_factory);

	(* G_OBJECT_CLASS (iog_file_chooser_parent_class)->finalize) (object);
}

static void
iog_file_chooser_class_init (IogFileChooserClass *klass)
{
	GObjectClass *object_class = (GObjectClass *) klass;

	object_class->finalize = iog_file_chooser_finalize;

	g_type_class_add_private (object_class, sizeof (IogFileChooserPrivate));
}

static void
iog_file_chooser_init (IogFileChooser *chooser)
{
	chooser->priv = IOG_FILE_CHOOSER_GET_PRIVATE (chooser);
}
...
save_response_cb (GtkDialog *dlg, gint id, gpointer data)
{
	...
	if (!format || !gdk_pixbuf_format_is_writable (format)) {
		...
		gtk_message_dialog_format_secondary_text (
						...
						"%s\n%s",
		 				_("Eye of GDE2 could not determine a supported writable file format based on the filename."),
...
static void
iog_file_chooser_add_filter (IogFileChooser *chooser)
{
	GSList *it;
	GSList *formats;
	...
	if (action == GTK_FILE_CHOOSER_ACTION_SAVE) {
		formats = iog_pixbuf_get_savable_formats ();
	}
	else {
 ...
 */
static void
set_preview_pixbuf (IogFileChooser *chooser, GdkPixbuf *pixbuf, goffset size)
{
	IogFileChooserPrivate *priv;
	int bytes;
	int pixels;
	...
	g_return_if_fail (IOG_IS_FILE_CHOOSER (chooser));
	...
	if (pixbuf != NULL) {
		 ...
		 * thumbnail is created through the gde2 libraries.
		 */
		if (g_ascii_strcasecmp (creator, "Gde2::ThumbnailFactory") == 0) {
			creator = "GDE2 Libs";
		}
#endif
	}
...
update_preview_cb (GtkFileChooser *file_chooser, gpointer data)
{
	...
	priv = IOG_FILE_CHOOSER (file_chooser)->priv;
	...
	if ((file_info != NULL) && (priv->thumb_factory != NULL)
		...
		mtime = g_file_info_get_attribute_uint64 (file_info,
							  G_FILE_ATTRIBUTE_TIME_MODIFIED);
		thumb_path = gde2_desktop_thumbnail_factory_lookup (priv->thumb_factory, uri, mtime);
		if (thumb_path == NULL) {
		...
		if (thumb_path != NULL && g_file_test (thumb_path, G_FILE_TEST_EXISTS)) {
			...
			set_preview_pixbuf (IOG_FILE_CHOOSER (file_chooser), pixbuf,
...
static void
iog_file_chooser_add_preview (GtkWidget *widget)
{
	IogFileChooserPrivate *priv;
	GtkWidget *vbox;

	priv = IOG_FILE_CHOOSER (widget)->priv;
	...
	priv->thumb_factory = gde2_desktop_thumbnail_factory_new (GDE2_DESKTOP_THUMBNAIL_SIZE_NORMAL);
...
GtkWidget *
iog_file_chooser_new (GtkFileChooserAction action)
{
	GtkWidget *chooser;
	gchar *title = NULL;

	chooser = g_object_new (IOG_TYPE_FILE_CHOOSER,
				"action", action,
	...
	if (action != GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER) {
		iog_file_chooser_add_filter (IOG_FILE_CHOOSER (chooser));
		iog_file_chooser_add_preview (chooser);
	}
...
GdkPixbufFormat *
iog_file_chooser_get_format (IogFileChooser *chooser)
{
	GtkFileFilter *filter;
	GdkPixbufFormat* format;

	g_return_val_if_fail (IOG_IS_FILE_CHOOSER (chooser), NULL);
