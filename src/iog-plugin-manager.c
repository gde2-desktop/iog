...
#define IOG_PLUGIN_MANAGER_GET_PRIVATE(object) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((object), IOG_TYPE_PLUGIN_MANAGER, IogPluginManagerPrivate))

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE (IogPluginManager, iog_plugin_manager, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE (IogPluginManager, iog_plugin_manager, GTK_TYPE_VBOX)
#endif

#define PLUGIN_MANAGER_NAME_TITLE   _("Plugin")
#define PLUGIN_MANAGER_ACTIVE_TITLE _("Enabled")

struct _IogPluginManagerPrivate {
	GtkWidget	*tree;
...
static IogPluginInfo *plugin_manager_get_selected_plugin (IogPluginManager *pm);
static void plugin_manager_toggle_active (GtkTreeIter *iter, GtkTreeModel *model);
static void iog_plugin_manager_finalize (GObject *object);

static void
iog_plugin_manager_class_init (IogPluginManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = iog_plugin_manager_finalize;

	g_type_class_add_private (object_class, sizeof (IogPluginManagerPrivate));
}
		 ...
		 IogPluginManager *pm)
{
	...
	pm->priv->about = g_object_new (GTK_TYPE_ABOUT_DIALOG,
		"program-name" , iog_plugin_engine_get_plugin_name (info),
		"copyright", iog_plugin_engine_get_plugin_copyright (info),
		"authors", iog_plugin_engine_get_plugin_authors (info),
		"comments", iog_plugin_engine_get_plugin_description (info),
		"website", iog_plugin_engine_get_plugin_website (info),
		"logo-icon-name", iog_plugin_engine_get_plugin_icon_name (info),
		NULL);
		     ...
		     IogPluginManager *pm)
{
	...
	iog_debug_message (DEBUG_PLUGINS, "Configuring: %s\n",
			   iog_plugin_engine_get_plugin_name (info));

	toplevel = GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET(pm)));

	iog_plugin_engine_configure_plugin (info, toplevel);

	iog_debug_message (DEBUG_PLUGINS, "Done");
}
				  ...
				  gpointer           data)
{
	...
	text = g_markup_printf_escaped ("<b>%s</b>\n%s",
					iog_plugin_engine_get_plugin_name (info),
					iog_plugin_engine_get_plugin_description (info));

	g_object_set (G_OBJECT (cell),
		      "markup", text,
		      "sensitive", iog_plugin_engine_plugin_is_available (info),
		      NULL);
				  ...
				  gpointer           data)
{
	...
	g_object_set (G_OBJECT (cell),
		      "icon-name", iog_plugin_engine_get_plugin_icon_name (info),
		      "sensitive", iog_plugin_engine_plugin_is_available (info),
		      NULL);
}
		   ...
		   gpointer     data)
{
	...
	gtk_widget_set_sensitive (GTK_WIDGET (pm->priv->configure_button),
				  (info != NULL) &&
				   iog_plugin_engine_plugin_is_configurable (info));
}
...
static void
plugin_manager_populate_lists (IogPluginManager *pm)
{
	const GList *plugins;
	GtkListStore *model;
	GtkTreeIter iter;
	...
	while (plugins) {
		IogPluginInfo *info;
		info = (IogPluginInfo *)plugins->data;

		gtk_list_store_append (model, &iter);
		gtk_list_store_set (model, &iter,
				    ACTIVE_COLUMN, iog_plugin_engine_plugin_is_active (info),
				    AVAILABLE_COLUMN, iog_plugin_engine_plugin_is_available (info),
				    INFO_COLUMN, info,
				    -1);
			   ...
			   gboolean      active)
{
	...
	if (active) {
		/* Activate the plugin */
		if (!iog_plugin_engine_activate_plugin (info)) {
			iog_debug_message (DEBUG_PLUGINS, "Could not activate %s.\n",
					   iog_plugin_engine_get_plugin_name (info));
		...
		if (!iog_plugin_engine_deactivate_plugin (info)) {
			iog_debug_message (DEBUG_PLUGINS, "Could not deactivate %s.\n",
					     iog_plugin_engine_get_plugin_name (info));
...
plugin_manager_get_selected_plugin (IogPluginManager *pm)
{
	IogPluginInfo *info = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
plugin_manager_set_active_all (IogPluginManager *pm,
			       gboolean            active)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (iog_plugin_engine_get_plugin_name (info), -1, G_NORMALIZE_ALL);
...
static GtkWidget *
create_tree_popup_menu (IogPluginManager *pm)
{
	GtkWidget *menu;
	GtkWidget *item;
	GtkWidget *image;
	...
	gtk_widget_set_sensitive (item,
				  iog_plugin_engine_plugin_is_available (info));
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item),
					iog_plugin_engine_plugin_is_active (info));
...
static void
tree_popup_menu_detach (IogPluginManager *pm,
			GtkMenu            *menu)
{
...
model_name_sort_func (GtkTreeModel *model,
		      ...
		      gpointer      user_data)
{
	IogPluginInfo *info1, *info2;

	gtk_tree_model_get (model, iter1, INFO_COLUMN, &info1, -1);
	gtk_tree_model_get (model, iter2, INFO_COLUMN, &info2, -1);

	return g_utf8_collate (iog_plugin_engine_get_plugin_name (info1),
			       iog_plugin_engine_get_plugin_name (info2));
}

static void
plugin_manager_construct_tree (IogPluginManager *pm)
{
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
	GtkListStore *model;
...
static void
iog_plugin_manager_init (IogPluginManager *pm)
{
	GtkWidget *label;
	GtkWidget *viewport;
	...
	pm->priv = IOG_PLUGIN_MANAGER_GET_PRIVATE (pm);
	...
	pm->priv->plugins = iog_plugin_engine_get_plugins_list ();
...
static void
iog_plugin_manager_finalize (GObject *object)
{
	IogPluginManager *pm = IOG_PLUGIN_MANAGER (object);

	if (pm->priv->popup_menu)
		gtk_widget_destroy (pm->priv->popup_menu);

	G_OBJECT_CLASS (iog_plugin_manager_parent_class)->finalize (object);
}

GtkWidget *
iog_plugin_manager_new (void)
{
	return g_object_new (IOG_TYPE_PLUGIN_MANAGER, NULL);
}
